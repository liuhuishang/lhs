...
(1)符号式编程:定义计算流程;将流程编译成可执行的程序；给定输入，调用编译好的程序执行。
命令式编程比符号式编程更容易使用。符号式编程比命令式编程运行效率更高，更易于移植。

（2）混合式编程：将命令式编程和符号式编程结合。

（3）Sequential的混合式编程：1.通过混合式编程加速2.序列化

2.异步计算

(1)通过后端异步处理:后端必须能够跟踪计算图中各个步骤之间的依赖关系。因此，不可能并行化相互依赖的操作。

(2)障碍器与阻塞器(3)改进计算

3.自动并行

（1）基于GPU的并行计算：将数据分配到两个变量上

（2）并行计算与通信：在不同的设备之间移动数据。例如![模型图片](/image/28.png)

4.硬件

（1）计算机：一个处理器（也被称为CPU），内存（随机访问存储，RAM）用于存储和检索计算结果，一个或多个以太网连接，高速扩展总线（PCIe）用于系统连接一个或多个GPU。持久性存储设备。

（2）内存：最基本的内存主要用于存储需要随时访问的数据。

（3）存储器：关键特性是 带宽（bandwidth）和 延迟（latency）1.硬盘驱动器2.固态驱动器3.云存储

（4)cpu:是任何计算机的核心,由处理器核心，总线，缓存组成![模型图片](/image/29.png)

（5）微体系结构：![模型图片](/image/30.png)

（6）矢量化：![模型图片](/image/31.png)

（7）缓存：寄存器，一级缓存，二级缓存，三级缓存

（8）gpu和其他加速卡：加速卡是为训练还是推断而优化的。处理器核心中添加向量处理单元可以显著提高吞吐量。

（9）网络和总线：pcle 以太网 交换机 nvlink

5.多GPU训练

（1）问题拆分：1.在多个GPU之间拆分网络。 也就是说，每个GPU将流入特定层的数据作为输入，跨多个后续层对数据进行处理，然后将数据发送到下一个GPU。 2.拆分层内的工作。 3.跨多个GPU对数据进行拆分。

（2)数据并行性:在任何一次训练迭代中，给定的随机的小批量样本都将被分成K个部分，并均匀地分配到GPU上；每个GPU根据分配给它的小批量子集，计算模型参数的损失和梯度；将K个GPU中的局部梯度聚合，以获得当前小批量的随机梯度；聚合梯度被重新分发到每个GPU中；每个GPU使用这个小批量随机梯度，来更新它所维护的完整的模型参数集。

（3）数据同步：首先，我们需要向多个设备分发参数并附加梯度（get_params）。 如果没有参数，就不可能在GPU上评估网络。 第二，需要跨多个设备对参数求和，也就是说，需要一个allreduce函数。

（4）数据分发：将一个小批量数据均匀地分布在多个GPU上。 

6.参数服务器

（1）数据并行训练：其中的关键是梯度的聚合需要在单个GPU（GPU 0）上完成，然后再将更新后的参数广播给所有GPU。![模型图片](/image/32.png)

（2）环同步（Ring Synchronization）：环同步对于p3和DGX-2服务器是最佳的，而对于其他服务器则未必。![模型图片](/image/33.png)

（3）多机训练：分层同步策略可以工作的很好。![模型图片](/image/34.png)

（4）键值存储：push（key，value）将特定的梯度值从工作节点发送到公共存储，在那里通过某种方式（例如，相加）来聚合值；pull（key，value）从公共存储中取得某种方式（例如，组合来自所有工作节点的梯度）的聚合值。：
